 ...
 * @author DaBin
 */
public class SignUp_ConsumerController implements Initializable {
    ...
    @FXML
    private TextField TextField_Uni;
    @FXML
    ...
    private void Push_BTN_SignUp(ActionEvent event) {
        ...
        String Uni = TextField_Uni.getText().toString();
        String Email = TextField_Email.getText().toString();

        //(1) 빈 칸이 모두 채워지지 않은 경우
        if (ID.equals("") || PW.equals("") || Name.equals("") || PhoneNum.equals("") || Uni.equals("") || Email.equals("")) {
            ...
            try {
                ...
                if (resultSet.next()) { // 중복된 ID가 존재한다면
                    ...
                    confirmBox(ID, PW, Name, PhoneNum, Uni, Email);
                }
    ...
    // 회원 가입 시 최종 확인
    public void confirmBox(String ID, String PW, String Name, String PhoneNum, String Uni, String Email) {
        try {
            ...
            if (result.get() == ButtonType.OK) {
                try { // 확인을 누를 경우 sql문을 보낸다.
                    ...
                    String sql = "insert into consumer (consumer_id, password, name, phonenumber, uni_name, Email, BeanAmount) values(?, ?, ?, ?, ?, ?,?)";
                    String Bean = "0"; // 처음 회원 가입시 Bean의 갯수는 무조건 0개
                    ...
                    preparedStatement.setString(5, Uni);
                    preparedStatement.setString(6, Email);
                    preparedStatement.setString(7, Bean);
